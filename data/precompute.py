...
Back-compat (single .npz; OK only with --workers 0):
...
def main():
    ...
    ap.add_argument("--out", type=str, default=None, help="legacy .npz (not mmap-friendly)")
    ...
    for fen in tqdm(iter_fens(args.positions), desc="Precomputing"):
        try:
            ...
            continue
        x = board_to_planes(board)            # (Cin, 8, 8)
